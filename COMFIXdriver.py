...
for line in lines:
    ...
    file_out.write(f"Rho_sez  =   {format(rho_sez[0], '.4f')} S    {format(rho_sez[1], '.4f')} E   {format(rho_sez[2], '.4f')} Z Mag =   {format(CF.mag(rho_sez),'.4f')} km \n")
    file_out.write(f"Drho_sez =       {format(drho_sez[0], '4.4f')} S     {format(drho_sez[1], '.4f')} E      {format(drho_sez[2], '.4f')} Z Mag =     {format(CF.mag(drho_sez),'.4f')} km/s \n")
    file_out.write(f"R_site   =   {format(Rsite[0], '.4f')} I   {format(Rsite[1], '.4f')} J   {format(Rsite[2], '.4f')} K Mag =   {format(CF.mag(Rsite),'.4f')} km \n")
    file_out.write(f"Rho_ijk  =     {format(rho_ijk[0], '.4f')} I   {format(rho_ijk[1], '.4f')} J   {format(rho_ijk[2], '.4f')} K Mag =   {format(CF.mag(rho_ijk),'.4f')} km \n")
    file_out.write(f"Drho_ijk =      {format(drho_ijk[0], '.4f')} I      {format(drho_ijk[1], '.4f')} J      {format(drho_ijk[2], '.4f')} K Mag =     {format(CF.mag(drho_ijk),'.4f')} km/s \n")
    file_out.write(f"R_ijk    =      {format(R[0], '.4f')} I   {format(R[1], '.4f')} J   {format(R[2], '.4f')} K Mag =  {format(CF.mag(R),'.4f')} km \n")
    file_out.write(f"V_ijk    =      {format(V[0], '.4f')} I      {format(V[1], '.4f')} J      {format(V[2], '.4f')} K Mag =      {format(CF.mag(V),'.4f')} km/s \n")
    file_out.write("------------------------------------------------------------------\n\n")
